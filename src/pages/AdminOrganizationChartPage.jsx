...
function OrgNode({ label, sublabel, highlight, children }) {
  return (
    <div className="flex flex-col items-center gap-0">
      ...
      >
        {/* Colors are set inline (not via Tailwind text-color classes) because the
            shared `.page-content p` rule outranks a plain utility class on <p>
            elements and was silently forcing every label back to body-gray. */}
        <p className="font-hind font-semibold text-[0.875rem] leading-snug"
          style={{ color: highlight ? '#ffffff' : college.primaryColor }}>
          {label}
        </p>
        {sublabel && (
          <p className="font-dm-sans text-[10px] mt-0.5"
