...
function YearButton({ label }) {
  return (
    <a
      href="#"
      className="font-montserrat font-semibold text-[13px] text-white text-center py-2.5 px-6 rounded-full hover:opacity-90 transition-opacity"
  ...
  return (
    <AcademicsLayout title="Academic Calendar">
        ...
        {RECENT_AYS.map((ay) => (
          <div key={ay} className="p-5 rounded-lg border border-gray-200 bg-white">
            <div className="flex items-center gap-2 mb-4">
              <span
                className="font-poppins font-bold text-[13px] px-3 py-0.5 rounded-full text-white"
      ...
      <div className="pt-6 border-t border-gray-200">
        <h3 className="font-poppins font-semibold text-[14px] mb-4 text-gray-500">Previous Years</h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-8 gap-y-2">
          {OLDER_AYS.flatMap((ay) => [
            <a key={`b-${ay}`} href="#" className="font-montserrat text-[13px] font-medium hover:underline" style={{ color: college.primaryColor }}>
              B.Tech A.Y. {ay}
            </a>,
            <a key={`m-${ay}`} href="#" className="font-montserrat text-[13px] font-medium hover:underline" style={{ color: college.primaryColor }}>
              M.Tech A.Y. {ay}
            </a>,
          ])}
        </div>
