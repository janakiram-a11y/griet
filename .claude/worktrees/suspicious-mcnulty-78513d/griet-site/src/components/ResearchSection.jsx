    ...
    <section className="w-full bg-[#5B1027] page-pad">
      <div className="flex flex-col gap-12">
        <div className="flex flex-col items-center gap-3">
          <p className="font-poppins font-bold text-[20px] leading-5 uppercase tracking-[1.4px] text-[#C32033] text-center">
        ...
        <div className="grid grid-cols-4 gap-8">
          {college.researchStats.map(({ count, label, desc }) => (
            ...
            >
              <span className="font-poppins font-bold text-[30px] leading-[38px] text-[#C32033]">{count}</span>
              <span className="font-poppins font-semibold text-[18px] leading-7 text-white">{label}</span>
              <span className="font-poppins font-normal text-[14px] leading-5 text-[#F3DAB2]">{desc}</span>
            </div>
          ))}
        </div>
      </div>
