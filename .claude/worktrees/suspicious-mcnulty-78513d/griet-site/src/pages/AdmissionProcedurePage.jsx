  ...
  return (
    <div className="flex-1 rounded-lg overflow-hidden border border-gray-200 bg-white shadow-sm">
      <div className="px-6 py-4 flex items-center gap-3" style={{ backgroundColor: college.primaryColor }}>
        <span className="font-poppins font-bold text-white text-[18px]">{title}</span>
        {badge && (
          <span className="ml-auto font-montserrat text-[11px] font-semibold px-2 py-0.5 rounded-full bg-white" style={{ color: college.primaryColor }}>
            {badge}
          </span>
        )}
      </div>
...
function Step({ n, children }) {
  return (
    <div className="flex items-start gap-3 mb-3">
      <div
        className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center font-poppins font-bold text-[12px] text-white mt-0.5"
        style={{ backgroundColor: college.accentColor }}
      >
        {n}
      </div>
      <p className="font-montserrat text-[13px] leading-relaxed text-gray-700">{children}</p>
    </div>
  );
}

function Body({ children }) {
  return <p className="font-montserrat text-[13px] leading-relaxed text-gray-700 mb-3">{children}</p>;
}

function SubLabel({ children }) {
  return (
    <p className="font-montserrat font-bold text-[12px] uppercase tracking-wide text-gray-400 mb-2 mt-4">{children}</p>
  );
}
...
export default function AdmissionProcedurePage() {
  return (
    <AdmissionsLayout title="Admission Procedure">
      ...
      {/* Eligibility note */}
      <p className="font-montserrat text-[13px] text-gray-500 italic mb-8">
      ...
      <div className="flex flex-col lg:flex-row gap-6 mb-10">
        ...
        <Panel title="M.Tech Programmes" badge="GATE / PGECET">
          ...
          <div className="mt-5 pt-4 border-t border-gray-100 space-y-2">
            <a href="#" style={{ color: college.primaryColor }} className="font-montserrat font-semibold text-[13px] underline hover:opacity-80 block">
              M.Tech Notification →
            </a>
            <a href="#" style={{ color: college.primaryColor }} className="font-montserrat font-semibold text-[13px] underline hover:opacity-80 block">
      ...
      <div className="rounded-lg p-5" style={{ backgroundColor: '#F3DAB2', border: `1px solid ${college.accentColor}55` }}>
        <p className="font-poppins font-semibold text-[14px] mb-1" style={{ color: college.primaryColor }}>
          AICTE Scholarship Note
        </p>
